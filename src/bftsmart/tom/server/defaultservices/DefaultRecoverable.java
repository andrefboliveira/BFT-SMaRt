 ...
 * @author Joao Sousa
 */
public abstract class DefaultRecoverable implements Recoverable, BatchExecutable, ServerJoiner {
...
//        byte[][] replies = new byte[0][];

		List<byte[]> transListApp = new ArrayList<byte[]>();
		ArrayList<MessageContext> ctxListApp = new ArrayList<>();


		List<byte[]> joinRequestList = new ArrayList<byte[]>();
		ArrayList<MessageContext> ctxjoinRequestList = new ArrayList<>();
...
//			if (controller.isCurrentViewMember(msgCtxs[i].getSender())) {
		...
		if (transListApp.size() > 0) {
			...
			repliesApp = normalExecution(transArrayApp, ctxArrayApp, noop);
		}
	...
	private byte[][] joinExecution(byte[][] commands, MessageContext[] msgCtxs, boolean noop) {
...
//	        	System.out.println("----THERE IS A SECOND HALF----");
	...
	private byte[][] normalExecution(byte[][] commands, MessageContext[] msgCtxs, boolean noop) {
		int cid = msgCtxs[msgCtxs.length - 1].getConsensusId();
